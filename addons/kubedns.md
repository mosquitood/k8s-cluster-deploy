...
已经修改好的 yaml 文件见：[dns](https://github.com/mosquitood/k8s-cluster-deploy/tree/master/addons/dns)。
...
+ 需要将 spec.clusterIP 设置为环境变量中变量 `CLUSTER_DNS_SVC_IP` 值，这个 IP 需要和 kubelet 的 `—cluster-dns` 参数值一致；
...
+ `--domain` 为变量 `CLUSTER_DNS_DOMAIN` 的值；
...
``` bash
$ pwd
/root/k8s-cluster-deploy/addons/dns
$ ls *.yaml
